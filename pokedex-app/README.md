...
### Как запустить проект pokedex-app

1. Скачайте приложение на компьютер.
2. Войдите в папку pokedex-app и откройте с помощью своего IDE.
3. Для запуска сервера db.json введите в терминал следующую команду: json-server --watch data/db.json --port 8000
4. Для запуска приложения введите в терминал следующую команду: npm start

Перечень всех необходимых npm-пакетов для работы приложения находится в файле package.json в свойстве dependencies.